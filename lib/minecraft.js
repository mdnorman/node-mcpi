...
class MinecraftClass {
  ...
  // commands
  /**
   * `mc.getBlockWithData(x, y, z)` -- Returns the block ID and data at the selected coordinates.
   *
   * @param {number} x
   * @param {number} y
   * @param {number} z
  ...
  getBlockWithData(x, y, z) {
    return this.sendReceive('world.getBlockWithData(' + x + ',' + y + ',' + z + ')').then(data => {
      ...
      return {
        blockId: Number(parts[0]),
        dataValue: Number(parts[1])
      };
    });
  };
}
...
Minecraft.prototype.setBlock = function (x, y, z, id, data) {
  const command = data ? this.send('world.setBlock(' + x + ',' + y + ',' + z + ',' + id + ',' + data + ')') : this.send('world.setBlock(' + x + ',' + y + ',' + z + ',' + id + ')');
  return command;
};
...
Minecraft.prototype.setBlocks = function (x1, y1, z1, x2, y2, z2, id, data) {
  const command = data ? this.send('world.setBlocks(' + x1 + ',' + y1 + ',' + z1 + ',' + x2 + ',' + y2 + ',' + z2 + ',' + id + ',' + data + ')') : this.send('world.setBlocks(' + x1 + ',' + y1 + ',' + z1 + ',' + x2 + ',' + y2 + ',' + z2 + ',' + id + ')');
  return command;
};
